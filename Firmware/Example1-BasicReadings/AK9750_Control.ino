...
status_t ak9750Setup(void)
{
  byte deviceID = 0;
  status_t returnError = readRegister(AK9750_WIA2, &deviceID);
...
{
  byte dummy = 0;
  readRegister(AK9750_ST2, &dummy); //Do nothing but read the register
}
...
void setMode(byte mode)
{
  ...
  //Read, mask set, write
  byte currentSettings = 0;
  readRegister(AK9750_ECNTL1, currentSettings);
...
status_t readRegister(byte location, byte* outputPointer)
{
  ...
  Wire.requestFrom(AK9750_ADDR, numBytes);
